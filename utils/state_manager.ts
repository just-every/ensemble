...
export class RequestContextWithState implements RequestContext {
    ...
    updateScore(key: string, score: number): void {
        const scores = this.stateManager.get('scores', {}) as Record<string, number>;
        scores[key] = score;
    ...
    disableModel(model: string, reason?: string): void {
        const disabled = this.stateManager.get('disabledModels', {}) as Record<string, string | boolean>;
    ...
    recordRequestTime(model: string, duration: number): void {
        const timings = this.stateManager.get('requestTimings', {}) as Record<string, number[]>;
        if (!timings[model]) {
            timings[model] = [];
        }
