...
import type { ProviderStreamEvent, AgentDefinition, AgentExportDefinition } from '../types/types.js';
...
describe('runAgentTool Event Context', () => {
    it('should add agent context to all streamed events', async () => {
        ...
        const result = await agentTool.function({ 
            prompt: 'Test prompt',
            goal: 'Test goal'
        });
        ...
        // parent_id should be the original agent's parent_id
        expect(agentStartEvent?.parent_id).toBe(mockAgent.parent_id);
        ...
        if (messageDeltas.length > 0) {
            ...
            expect(firstDelta.parent_id).toBe(mockAgent.parent_id);
        }
        ...
        if (messageComplete) {
            ...
            expect(messageComplete.parent_id).toBe(mockAgent.parent_id);
        }
        ...
        expect(agentDoneEvent?.parent_id).toBe(mockAgent.parent_id);
    });
    ...
    it('should preserve original event properties when adding context', async () => {
        ...
        if (messageDelta) {
            ...
            expect(messageDelta.parent_id).toBe(mockAgent.parent_id);
        }
    });
});